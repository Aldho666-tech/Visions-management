...
export default function HeroSection({ onExploreWork, onStartProject }: HeroSectionProps) {
  ...
  return (
    <section id="vision" className="relative min-h-screen flex flex-col justify-between overflow-hidden bg-[#171717]">
      {/* Background Video with Black Vignette */}
      ...
      {/* Center Ultra-Clean Headline & Action Button (Main Focus) */}
      ...
      {/* Bottom Area: Refined Compact Marquee & Info Bar (Zero Bottom Gap) */}
      <div className="relative z-10 w-full pb-3 pt-0">
        {/* Refined Balanced Running Text Scale */}
        <div className="w-full overflow-hidden select-none py-1.5 mb-1.5 border-y border-white/10 bg-black/40 backdrop-blur-sm">
          <motion.div
            ...
            className="flex items-center whitespace-nowrap space-x-6 sm:space-x-8 w-max py-0.5"
          >
            {[...marqueeItems, ...marqueeItems, ...marqueeItems].map((item, index) => (
              <span
                key={index}
                className={`font-heading font-light text-3xl sm:text-4xl md:text-5xl lg:text-6xl leading-none uppercase tracking-tight ${
