"use client";

import React from "react";
import Link from "next/link";

interface FooterProps {
  onOpenProjectModal?: () => void;
  onOpenTalentModal?: () => void;
}

export default function Footer({ onOpenProjectModal, onOpenTalentModal }: FooterProps) {
  return (
    <footer className="bg-[#171717] text-[#F8F7F4] pt-16 sm:pt-20 pb-10 border-t border-white/10 font-heading font-light">
      <div className="max-w-[1440px] mx-auto px-6 sm:px-10 lg:px-14">
        {/* Top Layout: Brand Column, 2x3 Links Group, and Contact */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-10 lg:gap-12 pb-12 sm:pb-16">
          {/* Column 1: Brand Logo & Social Icons */}
          <div className="lg:col-span-5 flex flex-col justify-between space-y-8">
            <div className="space-y-3">
              <Link href="/" className="flex items-center space-x-3.5 group">
                <img
                  src="/logo-vm.svg"
                  alt="VM Icon"
                  className="h-8 sm:h-10 w-auto object-contain filter group-hover:brightness-125 transition-all drop-shadow-[0_0_15px_rgba(199,155,99,0.3)] shrink-0"
                />
                <span className="font-heading font-light tracking-[0.3em] sm:tracking-[0.4em] text-base sm:text-lg lg:text-xl uppercase text-white group-hover:text-[#C79B63] transition-colors leading-none whitespace-nowrap">
                  VISIONS MANAGEMENT
                </span>
              </Link>
              <div className="text-xs text-neutral-400 font-light tracking-widest uppercase pt-1">
                Connecting Your Business
              </div>
            </div>

            {/* Social Icons: Facebook, Instagram, LinkedIn */}
            <div className="flex items-center space-x-4 text-neutral-300 pt-4">
              <a
                href="https://facebook.com"
                target="_blank"
                rel="noreferrer"
                className="hover:text-[#C79B63] transition-colors p-2.5 bg-white/5 hover:bg-white/10 rounded-full border border-white/10"
                aria-label="Facebook"
              >
                <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24">
                  <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                </svg>
              </a>
              <a
                href="https://instagram.com"
                target="_blank"
                rel="noreferrer"
                className="hover:text-[#C79B63] transition-colors p-2.5 bg-white/5 hover:bg-white/10 rounded-full border border-white/10"
                aria-label="Instagram"
              >
                <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24">
                  <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/>
                </svg>
              </a>
              <a
                href="https://linkedin.com"
                target="_blank"
                rel="noreferrer"
                className="hover:text-[#C79B63] transition-colors p-2.5 bg-white/5 hover:bg-white/10 rounded-full border border-white/10"
                aria-label="LinkedIn"
              >
                <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24">
                  <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/>
                </svg>
              </a>
            </div>
          </div>

          {/* Column 2: Single Group of 6 Links in 2 Rows x 3 Columns */}
          <div className="lg:col-span-4">
            <h4 className="text-xs font-mono uppercase tracking-[0.25em] text-[#C79B63] mb-4">
              Navigation
            </h4>
            <div className="grid grid-cols-3 gap-y-3 gap-x-4 text-sm sm:text-base font-light text-[#F8F7F4]">
              {/* Row 1 */}
              <Link href="/" className="hover:text-[#C79B63] transition-colors">
                Home
              </Link>
              <Link href="/about" className="hover:text-[#C79B63] transition-colors">
                About
              </Link>
              <Link href="/services" className="hover:text-[#C79B63] transition-colors">
                Services
              </Link>

              {/* Row 2 */}
              <Link href="/partners" className="hover:text-[#C79B63] transition-colors">
                Partners
              </Link>
              <Link href="/news" className="hover:text-[#C79B63] transition-colors">
                News
              </Link>
              <Link href="/contact" className="hover:text-[#C79B63] transition-colors">
                Contact
              </Link>
            </div>
          </div>

          {/* Column 3: Contact Block (No Physical Address) */}
          <div className="lg:col-span-3 space-y-3 text-sm sm:text-base font-light text-[#F8F7F4]">
            <h4 className="text-xs font-mono uppercase tracking-[0.25em] text-[#C79B63] mb-4">
              Contact Us
            </h4>
            <div>
              <a
                href="tel:+6281234567890"
                className="hover:text-[#C79B63] transition-colors block tracking-wide text-neutral-200"
              >
                +62 812 3456 7890
              </a>
            </div>
            <div>
              <a
                href="mailto:hello@visionsmanagement.id"
                className="hover:text-[#C79B63] transition-colors block tracking-wide text-neutral-200"
              >
                hello@visionsmanagement.id
              </a>
            </div>

            <div className="text-xs sm:text-sm text-neutral-400 font-light leading-relaxed pt-2">
              <p className="text-white font-light">VISIONS MANAGEMENT Partnership Agency</p>
              <p className="text-neutral-400">by VISIONS ENTERPRISE</p>
            </div>
          </div>
        </div>

        {/* Bottom Horizontal Line */}
        <div className="border-t border-white/20 my-8 sm:my-10" />

        {/* Bottom Copyright Bar & Policies */}
        <div className="flex flex-col sm:flex-row items-center justify-between text-xs sm:text-sm font-light text-neutral-400 space-y-4 sm:space-y-0">
          <div>
            <span>© VISIONS MANAGEMENT Partnership Agency by VISIONS ENTERPRISE</span>
          </div>

          <div className="flex items-center space-x-6">
            <Link href="/policies-and-terms" className="hover:text-white transition-colors">
              Policies and Terms
            </Link>
          </div>
        </div>
      </div>
    </footer>
  );
}
